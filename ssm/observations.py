...
# For DMDM dataset's GLM-HMM
class InputDrivenObservationsMultinominal(Observations):
    ...
    def __init__(self, K, D, M=0, C=2, prior_mean=0, prior_sigma=1000):
        ...
        matrix (prior_sigma is on diagonal)
        """
        super(InputDrivenObservationsMultinominal, self).__init__(K, D, M)
        self.C = C
        self.M = M
        self.D = D
        self.K = K
    ...
    def log_likelihoods(self, data, input, mask, tag):
        time_dependent_logits = self.calculate_logits(input)
        assert self.D == 1, "InputDrivenObservationsMultinominal written for D = 1!"
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.categorical_logpdf(data[:, None, :],
                                        time_dependent_logits[:, :, None, :],
                                        mask=mask[:, None, :])
